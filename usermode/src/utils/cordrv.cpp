...
uint64_t CorDrv::GetSystemEprocessVA() {
    ...
    while (NtQuerySystemInformation(SystemExtendedHandleInformation, buffer, bufferLength, &returnSize) != 0) {
        bufferLength = returnSize + 0x1000;
        free(buffer);
    ...
    if (handleInfo) {
        for (ULONG_PTR i = 0; i < handleInfo->NumberOfHandles; i++) {
            ...
            // System process -> pid 4, handle 4 is guaranteed to be its EPROCESS object
            if (h.UniqueProcessId == 4 && h.HandleValue == 4) {
...
uint64_t CorDrv::FindProcessDTB(DWORD Pid) {
    ...
    if (!systemEprocessVA) {
        printf("failed to get system EPROCESS\n");
        return 0;
    }
    ...
    do {
        ...
        ReadPhysicalMemory(eprocessPhys + EProcess::ActiveProcessLinks, &currentFlink, sizeof(currentFlink));
    ...
    } while (currentFlink != listHeadVA && currentFlink != 0);
...
bool CorDrv::ReadProcessMemory(uint64_t DTB, uint64_t VirtualAddress, void* Buffer, size_t Size) {
    ...
    while (remaining > 0) {
        ...
        size_t chunk = std::min(remaining, (size_t)(PAGE_4KB - (va & 0xFFF)));

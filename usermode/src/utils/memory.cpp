...
bool c_memory::setup()
{
	...
	if (!m_drv.Initialize()) {
		LOG_ERROR("failed to initialize CorDrv. Ensure CORMEM.SYS is loaded.");
		return false;
	}

	m_process_dtb = m_drv.FindProcessDTB(this->m_id);
	if (!m_process_dtb) {
		LOG_ERROR("failed to find DTB for process %u", this->m_id);
		return false;
	}
